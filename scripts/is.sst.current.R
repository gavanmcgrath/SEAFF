...
#' 
is.sst.current <- function(fid="./data/SST"){
  nc.fid <- paste(fid,"/sst.mnmean.nc",sep="")
  if (file.exists(nc.fid)) {
    #read file info at noaa
    require(curl)
    con <- curl("ftp://ftp.cdc.noaa.gov/Datasets/noaa.ersst/")
    dat <- readLines(con)
    close(con)
    datename <- grep("sst.mnmean.v4.nc",dat,value=TRUE)
